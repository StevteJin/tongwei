...
export default class SmartSorting extends Component {
    ...
    LoadDta(){
        fetch("http://121.196.181.211:8301/largescreen/largeScreen/getScreenData/sorting")
        .then(res => res.json())
        .then(data => {
            console.log(data,"接口请求成功")
            console.log(data.largeScreenModules,"分类")
            ...
            console.log(spliceData111,66666666666666666)
            console.log(spliceData,'分拣情况么' ) // 对这个数据遍历插入
           var spliceData1=data.largeScreenModules[0].largeScreenModuleDetailes.slice(9,15)
            console.log(spliceData1,'我丢哪里看不懂')
           var arrter1 = [];
           for(var i=0;i<spliceData.length;i+=3){
               arrter1.push(spliceData.slice(i,i+3));
           }
       
           var title=[]
        // 我带外面吃
           for(var i=0;i<spliceData1.length;){
                if(i%2==0){
                    console.log(i,111)
        ...
        // 当日入库
          ...
          fjx.forEach((element)=>{
                if(element.moduleDesc === "科盛异形烟线" || element.moduleDesc === "科盛异形烟线1" || element.moduleDesc === "兰剑分拣线" || element.moduleDesc === "兰剑分拣线1" || element.moduleDesc === "中溢手工线"){
                    console.log(element.largeScreenModuleDetailes,'ddddddddddddd')
           ...
           var arrter1 = [];
           for(var i=0;i<spliceData1.length;i+=3){
               arrter1.push(spliceData1.slice(i,i+3));
           }
            console.log(arrter1,"每三项分割")
            console.log(spliceData,"总数量")
            console.log(commoatin)
    ...
    render() {
        const {widthjd,TrueData,fjsl,fjsl1,fjsl2,dayKu}=this.state;
        return (
                                ...
                                fjsl1.length>0&&fjsl1.map((item,index)=>{
                                    ...
                                    return(
                                            <li>
                                                ...
                                                <p><span>{item[2].keyDesc}</span><span><i style={{width:`${widthjd}px`}}></i></span><span>{item[2].value}{item[2].unitDesc}</span></p>
                                            </li>
                                    )
                                })
                                // fjsl1.length>0&&fjsl1.map((item,index)=>{
